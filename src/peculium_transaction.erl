...
hash(#bitcoin_tx_message { version = Version, transaction_inputs = Inputs, transaction_outputs = Outputs, lock_time = LockTime }) ->
    ...
    InputsBin = lists:map(fun peculium_bitcoin_protocol_types:transaction_input/1, Inputs),
    {ok, OutputsLength} = t:var_int(length(Outputs)),
    OutputsBin = lists:map(fun peculium_bitcoin_protocol_types:transaction_output/1, Outputs),
